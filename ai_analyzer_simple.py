...
"""
AI 深度分析模块
基于 Ollama qwen3.5:9b 大模型进行深度传播分析
"""
...
def query_ollama(prompt: str, timeout: int = 180) -> str:
    """调用 Ollama API（通过 lm_client）"""
    result = call_lm(prompt, timeout=timeout)
    if result.get("success"):
        return result.get("content", "").strip()
    return result.get("error", "AI服务暂时不可用")
